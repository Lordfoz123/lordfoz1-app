import StopTrackingSheet, { StopReasonKey } from '@/components/StopTrackingSheet';
...
import { recordTrackingStop } from '@/services/trackingEvents';
...
export default function HomeScreen() {
  ...
  const uid = useMemo(() => user?.uid ?? currentUserId ?? null, [user?.uid, currentUserId]);
  const { currentLocation } = useRealTimeLocation(isTracking && uid ? uid : null);
  const { workStatus, isInWorkHours, currentSchedule, timeUntilNextChange } = useWorkSchedule(uid ?? undefined);
  ...
  // Limpieza de error GPS automática
  useEffect(() => {
    if (gpsError) {
  ...
  // Batería: lectura inicial + listeners (sin polling periódico)
  useEffect(() => {
  ...
  // Animación de pulso mientras tracking está activo
  useEffect(() => {
  ...
  // Animaciones de header y saludo
  const headerOpacity = scrollY.interpolate({ inputRange: [0, 100], outputRange: [0, 1], extrapolate: 'clamp' });
  const greetingOpacity = scrollY.interpolate({ inputRange: [0, 50], outputRange: [1, 0], extrapolate: 'clamp' });
  ...
  // Toggle tracking con permiso guiado y motivo de parada
  const handleTrackingToggle = async () => {
    const now = Date.now();
    if (now - lastToggleRef.current < 800) return; // debounce
    ...
    const ok = await ensureLocationPermissions({ requireBackground: true, appName: 'C&M' });
    if (!ok) return;
    ...
    if (success) {
      ...
      Alert.alert('No se pudo activar el GPS', 'Revisa los permisos de ubicación (incluido en segundo plano).', [
  ...
  // Confirmar motivo de parada: registramos evento y detenemos
  const confirmStopTracking = async (reason: StopReasonKey, note?: string) => {
    await recordTrackingStop({
      userId: uid ?? null,
      reason,
      note,
      battery: {
  ...
  // Estado visual según horario/seguimiento
  const getWorkStatusInfo = () => {
    const isOvertime = isTracking && !isInWorkHours;
    if (isTracking) {
      if (isOvertime) {
        return { color: '#8B5CF6', status: 'GPS Activo', details: 'Horas extra • Tracking manual', icon: 'time' as const };
      }
      switch (workStatus) {
        case 'WORKING':
          return {
  ...
  const activeRoute = useMemo(() => routes.find((r: any) => r.status === 'in-progress'), [routes]);
  const pendingRoutes = useMemo(() => routes.filter((r: any) => r.status === 'pending'), [routes]);
  const completedToday = useMemo(() => {
    const todayStr = new Date().toDateString();
    return routes.filter((r: any) => {
      if (r.status !== 'completed' || !r?.updatedAt) return false;
  ...
  const todayStats = useMemo(() => {
    const points = completedToday.reduce((sum: number, r: any) => sum + (r.completedPoints || 0), 0);
    const totalPoints =
      routes.filter((r: any) => r.status !== 'completed').reduce((sum: number, r: any) => sum + (r.totalPoints || 0), 0) +
      completedToday.reduce((sum: number, r: any) => sum + (r.totalPoints || 0), 0);
    const distance = completedToday.reduce((sum: number, r: any) => sum + (r.totalDistance || 0), 0);
    const time = completedToday.reduce((sum: number, r: any) => sum + (r.estimatedTime || 0), 0);
    const completed = completedToday.length;
    const pending = pendingRoutes.length;
    const inProgress = activeRoute ? 1 : 0;
    return { points, totalPoints, distance: distance.toFixed(1), time, completed, pending, inProgress };
  }, [routes, completedToday, pendingRoutes, activeRoute]);

  const dayProgress = useMemo(
    () => (todayStats.totalPoints > 0 ? Math.round((todayStats.points / todayStats.totalPoints) * 100) : 0),
    [todayStats],
  );
  ...
  const formatTimeAgo = (pastTs: number) => {
    const seconds = Math.floor((Date.now() - pastTs) / 1000);
  ...
  const getBatteryIcon = () => {
    if (batteryLevel === null) return 'battery-half-outline' as const;
    if (batteryState === Battery.BatteryState.CHARGING) return 'battery-charging' as const;
  ...
  const getBatteryColor = () => {
    ...
    if (batteryState === Battery.BatteryState.CHARGING) return '#4CAF50';
  ...
  // Chips bonitos en lugar de logs crudos
  const LocationChip = () => {
  ...
  const SyncChip = () => {
    if (!isTracking || !lastUpdate) return null;
    const alias = uid ? String(uid).slice(0, 6) : '—';
    return (
      <View style={[styles.chip, { backgroundColor: 'rgba(59,130,246,0.08)', borderColor: 'rgba(59,130,246,0.25)' }]}>
        <Ionicons name="refresh" size={14} color="#3B82F6" />
        <Text style={[styles.chipText, { color: '#1F2937' }]}>
          Sync {formatTimeAgo(lastUpdate)} • {alias}
        </Text>
      </View>
    );
  };
  ...
  return (
    <View style={[styles.container, isDark && styles.containerDark]}>
      ...
      <Animated.View style={[styles.headerWrapper, { opacity: headerOpacity }]}>
        <BlurView intensity={80} tint={isDark ? 'dark' : 'light'} style={[styles.headerBlur, isDark && styles.headerBlurDark]}>
        ...
        <Animated.View style={styles.errorBanner}>
          <View style={styles.errorContent}>
            ...
            <TouchableOpacity
              ...
              accessibilityLabel="Cerrar error de GPS"
            >
      ...
      >
        ...
        <Animated.View style={[styles.greetingSection, { opacity: greetingOpacity }]}>
          <View style={styles.greetingHeader}>
            <View style={styles.greetingLeft}>
              <Text style={[styles.greetingSmall, isDark && styles.textSecondaryDark]}>{getGreeting()}</Text>
              ...
              <View style={styles.dateTimeRow}>
                <View style={styles.dateTimeItem}>
                  ...
                  <Text style={[styles.dateTimeText, isDark && styles.textSecondaryDark]}>{formatDate()}</Text>
                </View>
                <View style={styles.dateTimeItem}>
                  <Ionicons name="time-outline" size={16} color={isDark ? '#999' : '#666'} />
                  <Text style={[styles.dateTimeText, isDark && styles.textSecondaryDark]}>{formatTime()}</Text>
                </View>
                {batteryLevel !== null && (
                  <View style={styles.dateTimeItem}>
                    <Ionicons name={getBatteryIcon()} size={16} color={getBatteryColor()} />
                    <Text style={[styles.dateTimeText, { color: getBatteryColor() }]}>{batteryLevel}%</Text>
                  </View>
                )}
              </View>
            </View>
        ...
        <View style={styles.section}>
          <View style={[styles.serviceCard, isDark && styles.cardDark]}>
            <View style={styles.serviceHeader}>
              ...
              <Text style={[styles.serviceTitle, isDark && styles.textDark]}>Estado del Servicio</Text>
              {isLoading && <ActivityIndicator size="small" color={workStatusInfo.color} style={{ marginLeft: 8 }} />}
            </View>

            {/* Área de control GPS como botón */}
            <TouchableOpacity
            ...
            >
              <View style={styles.gpsLeftSide}>
                <Animated.View
                  style={[
                    styles.gpsPulse,
                    { backgroundColor: `${workStatusInfo.color}20`, transform: [{ scale: pulseAnim }] },
                  ]}
                >
                ...
                <View style={styles.gpsTextContainer}>
                  <View style={styles.gpsStatusRow}>
                    ...
                    <Text style={[styles.gpsStatus, isDark && styles.textDark]}>{workStatusInfo.status}</Text>
                  </View>

                  <Text style={[styles.gpsDetails, isDark && styles.textSecondaryDark]}>{workStatusInfo.details}</Text>

                  {/* Chips de ubicación y sync */}
              ...
              {/* Botón visual de play/stop solo decorativo (evita eventos anidados) */}
              <View
                style={[
              ...
              >
                {isLoading ? (
                  ...
                  <Ionicons name={isTracking ? 'stop' : 'play'} size={16} color="white" />
                )}
              </View>
            </TouchableOpacity>
            ...
            {timeUntilNextChange && (
              <View style={styles.timeInfo}>
                ...
                <Text style={[styles.timeInfoText, { color: workStatusInfo.color }]}>{timeUntilNextChange}</Text>
              </View>
            )}

            <View style={styles.progressSection}>
              <View style={styles.progressHeader}>
                <Text style={[styles.progressTitle, isDark && styles.textSecondaryDark]}>Progreso del Día</Text>
        ...
        <View style={styles.section}>
          <View style={styles.metricsRow}>
            <View style={[styles.metricCard, isDark && styles.cardDark]}>
              ...
              <Text style={[styles.metricValue, isDark && styles.textDark]}>{todayStats.distance} km</Text>
              <Text style={[styles.metricLabel, isDark && styles.textSecondaryDark]}>Recorridos Hoy</Text>
            </View>
            ...
            <View style={[styles.metricCard, isDark && styles.cardDark]}>
              ...
              <Text style={[styles.metricLabel, isDark && styles.textSecondaryDark]}>Tiempo Activo</Text>
            </View>
          </View>
        </View>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <View style={styles.sectionTitleWithIcon}>
                ...
                <Text
                  style={[styles.sectionTitle, isDark && styles.textDark, { marginBottom: 0, marginLeft: 8 }]}
                >
                  Ruta Actual
                </Text>
              </View>
            ...
            >
              <View style={styles.activeRouteHeader}>
                <Text style={[styles.activeRouteTitle, isDark && styles.textDark]}>{activeRoute.name}</Text>
                <View style={styles.priorityBadgeContainer}>
                  ...
                  />
                  <Text
                    style={[styles.priorityBadgeText, { color: getPriorityColor(activeRoute.priority) }]}
                  >
                    {activeRoute.priority === 'high'
                      ? 'Alta'
                      : activeRoute.priority === 'medium'
                      ? 'Media'
                      : 'Baja'}
                  </Text>
                </View>
              ...
              <View style={styles.progressContainer}>
                <View style={styles.progressBar}>
                  <View
                    style={[
                      ...
                      {
                        width: `${
                          (activeRoute.completedPoints / Math.max(1, activeRoute.totalPoints)) * 100
                        }%`,
                      },
                    ]}
                  />
                ...
                <Text style={[styles.progressText, isDark && styles.textSecondaryDark]}>
                  {Math.round((activeRoute.completedPoints / Math.max(1, activeRoute.totalPoints)) * 100)}%
                </Text>
              </View>
        ...
        {pendingRoutes.length > 0 && (
          <View style={styles.section}>
            ...
            {pendingRoutes.slice(0, 3).map((route: any) => (
              <TouchableOpacity
                key={route.id}
              ...
              >
                <View style={styles.routeCardHeader}>
                  <View style={styles.routeCardLeft}>
                    <View
                      style={[styles.priorityIndicator, { backgroundColor: getPriorityColor(route.priority) }]}
                    />
            ...
            {pendingRoutes.length > 3 && (
              <TouchableOpacity style={styles.viewAllButton} onPress={() => router.push('/routes')}>
        ...
        {!loading && routes.length === 0 && (
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptyText, isDark && styles.textDark]}>No tienes rutas de monitoreo asignadas</Text>
        ...
        {loading && (
          <View style={styles.loadingContainer}>
            ...
            <Text style={[styles.loadingText, isDark && styles.textSecondaryDark]}>Cargando rutas...</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  headerBlurDark: { borderBottomColor: 'rgba(255, 255, 255, 0.1)' },
  ...
  greetingLeft: { flex: 1 },
  greetingSmall: { fontSize: 14, color: '#666', marginBottom: 4 },
  greeting: { fontSize: 40, fontWeight: 'bold', color: '#000', marginBottom: 12 },
  dateTimeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 16 },
  dateTimeItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  dateTimeText: { fontSize: 14, color: '#666' },

  section: { marginBottom: 24 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  sectionTitleWithIcon: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  sectionTitle: { fontSize: 20, fontWeight: 'bold', color: '#000', marginBottom: 12 },
  ...
  serviceHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 20, gap: 8 },
  serviceTitle: { fontSize: 18, fontWeight: '700', color: '#000' },
  ...
  gpsLeftSide: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  gpsPulse: {
    width: 80,
    height: 80,
    borderRadius: 40,
  ...
  gpsInner: { width: 60, height: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center' },
  gpsDot: {
    width: 32,
    height: 32,
    borderRadius: 16,
  ...
  gpsStatusRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  gpsActiveDot: { width: 8, height: 8, borderRadius: 4, marginRight: 8 },
  gpsStatus: { fontSize: 18, fontWeight: '700', color: '#000' },
  gpsDetails: { fontSize: 14, color: '#666', marginBottom: 8 },
  ...
  scheduleText: { fontSize: 15, color: '#666' },
  ...
  batteryContainerDark: { backgroundColor: 'rgba(255,255,255,0.1)' },
  ...
  timeInfoText: { fontSize: 13, fontWeight: '600' },

  progressSection: { marginTop: 4 },
  progressHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  progressTitle: { fontSize: 14, color: '#666' },
  progressPercentage: { fontSize: 20, fontWeight: '700', color: '#4CAF50' },
  ...
  // Resumen rápido
  quickSummary: { flexDirection: 'row', gap: 12 },
  summaryCard: {
    flex: 1,
    padding: 20,
    borderRadius: 16,
  ...
  summaryCardPending: { backgroundColor: '#E3F2FD' },
  summaryCardProgress: { backgroundColor: '#FFF3E0' },
  summaryCardCompleted: { backgroundColor: '#E8F5E9' },
  summaryNumber: { fontSize: 32, fontWeight: 'bold', color: '#333', marginTop: 12, marginBottom: 4 },
  ...
  metricValue: { fontSize: 22, fontWeight: '700', color: '#000', marginTop: 12, marginBottom: 4 },
  metricLabel: { fontSize: 13, color: '#666', textAlign: 'center' },
  ...
  activeRouteHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  activeRouteTitle: { fontSize: 20, fontWeight: '700', color: '#000', flex: 1 },
  ...
  progressText: { fontSize: 14, fontWeight: '600', color: '#666', minWidth: 40, textAlign: 'right' },

  routeMetrics: { flexDirection: 'row', gap: 16, marginBottom: 16 },
  metricItem: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  metricText: { fontSize: 14, color: '#666' },
  metricTextSmall: { fontSize: 12, color: '#999' },
  ...
  priorityIndicator: { width: 4, height: 24, borderRadius: 2, marginRight: 12 },
  routeCardTitle: { fontSize: 16, fontWeight: '600', color: '#000', flex: 1 },
  routeCardMetrics: { flexDirection: 'row', gap: 12, marginLeft: 16 },

  viewAllButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12, marginTop: 4 },
  viewAllText: { fontSize: 15, fontWeight: '600', color: '#4CAF50', marginRight: 4 },
  ...
  seedButtonText: { color: '#fff', fontSize: 16, fontWeight: '600' },
